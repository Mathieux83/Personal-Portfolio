					...
					<div key={id}>
						<div className='lg:min-h-130 h-100 flex items-center justify-center sm:w-96 w-[80vw]'>
							<CardContainer className='inter-var '>
								<CardBody className="relative group/card hover:shadow-2xl hover:shadow-emerald-500/10 bg-[url('/bg.png')] bg-cover border-white/10 w-auto sm:w-120 h-auto rounded-xl p-6 border">
									<CardItem
										translateZ='50'
										className='font-bold lg:text-xl md:text-md text-base line-clamp-1'
									>
										{/* {title} */}
										{id === 1 && <div>{t("project1.title")}</div>}
										{id === 2 && <div>{t("project2.title")}</div>}
									</CardItem>
									<CardItem
										// as='p'
										translateZ='60'
									...
									>
										{/* {des} */}
										{id === 1 && <div>{t("project1.description")}</div>}
										{id === 2 && <div>{t("project2.description")}</div>}
									</CardItem>
									...
									<div className='flex justify-between items-center mt-2 mb-3  '>
										...
										>
											{iconLists.map((icon, id) => (
												<div
													key={id}
													className='border border-white/20 rounded-full bg-black/70 p-2 flex justify-center items-center lg:w-10 lg:h-10 h-8 w-8'
